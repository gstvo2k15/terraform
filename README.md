...
Manipulate state only through TF commands (never touch directly file).<br>
Created by `terraform apply` command.

2.- Always setup a shared remote state:
<br>Terraform supports storing state in
like S3, Azure Blob Storage, Google Cloud Storage.

3.- Use state locking file for `terraform.tfstate` previous configure in your store backend.

4.- Losing your state file needs to be solved created regular backups of `terraform.tfstate` file.

5.- Usually you have multiple environments so you need to have multiple `terraform.tfstate` files.

6.- Important to share the TF code via git repository.

7.- CI for Terraform Code.

8.- Apply Infra changes ONLY throught CD pipeline.
